...
const myContentByClassName = document.querySelector('.content')
```
...
Som ett alternativ till att hämta element med klassnamn kan vi använda oss av `querySelectorAll()`. Där returneras en statisk nodelista där alla element med den valda *css selectorn* finns. Om vi tar ovan exempel igen men hämtar med querySelectorAll istället behöver vi bara ändra raden som hämtar elementen: `let allDivs = document.querySelectorAll('.content')`.
