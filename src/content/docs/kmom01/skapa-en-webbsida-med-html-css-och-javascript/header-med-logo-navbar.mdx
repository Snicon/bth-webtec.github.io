...
body {
  margin: 0; /* Removes the browser's default 8px margin around the page */
}
```
...
.siteheader nav a {
  ...
  border-bottom: 2px solid transparent; /* Reserve space for the hover underline so nothing jumps */
  transition: color 0.3s, border-bottom-color 0.3s; /* Add a smooth transition for color and border changes on hover */
}

/* Hover effect for navigation links */
.siteheader nav a:hover {
  color: #1f6f46;                         /* Darker forest green: passes WCAG AA (4.5:1) text contrast on the light green header background */
  border-bottom-color: #2e8b57;           /* Reveal the forest green underline when hovered */
}
```
